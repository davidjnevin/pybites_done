...
def most_popular_characters(characters=characters, top=5):
    ...
    """
    pop_char = [
        (character.name, int(character.appearances))
        for character in characters
        if character.appearances
    ][:top]

    return [character[0] for character in pop_char]
...
def max_and_min_years_new_characters(characters=characters):
    ...
    """
    new_char_years = Counter(
        character.year for character in characters if character.year
    )
    return (new_char_years.most_common()[0][0], new_char_years.most_common()[-1][0])
...
def get_percentage_female_characters(characters=characters):
    ...
    """
    all_genders = Counter(character.sex for character in characters if character.sex)
    # print(all_genders)
    females = all_genders["Female Characters"]
    total = sum(all_genders.values())

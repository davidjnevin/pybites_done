...
def get_movie_longest_runtime():
    ...
    root = get_tree()
    movie_list_dict = defaultdict()
    for movie in root.findall("movie"):
        movie_list_dict[movie.get("title")] = int((movie.get("runtime")).split(" ")[0])
    max_time = max(movie_list_dict.values())

    return [key for key in movie_list_dict if movie_list_dict[key] == max_time][0]
